...
| **Son güncelleme** | Nisan 2026 (kod tabanıyla uyumlu — i18n genişletildi, dil sheet’i, güvenlik/perf kuralları tazelendi) |
...
app/                    # Ekranlar ve navigasyon (expo-router)
  ...
  categories.tsx, goal-settings.tsx, notifications.tsx, …
...
| `vendors` | Satıcı adı, opsiyonel logo |
...
| `settings` | Anahtar-değer (tema, dil, bildirim durumu); `gemini_api_key` artık SQLite'da **değil**, OS keychain'de |
...
| **Tarayıcı** | Kamera/galeri, Gemini fiş önizleme, kaydet / düzenle, **şüşevar** kayıt butonu |
| **Analiz** | Sürükle-bırak kart sırası, bar/donut, heatmap, satıcı donut + ürün lejantı (Dashboard ile uyumlu animasyon), mikro analiz |
| **Ayarlar** | Bütçe (aylık + geçmiş), API anahtarı, tema (manuel + auto schedule), **dil (bottom-sheet picker)**, kategoriler, hedef (özellik anahtarı), satıcı yönetimi, **tarih aralığı bazlı yedekleme (export/import)** — §5.3 |
| **Harcama / ürün** | `add-expense`, `edit-items` (satır indirimi gösterimi, şüşevar) |
| **Bildirimler** | `app/notifications.tsx` + `NotificationsContext` + `src/notifications/*` (feed, kural motoru, sessize alma türleri) |
...
- **Onay:** geri yükleme her zaman `GlassDeleteModal` ile kullanıcıdan açık onay alır. Başarı / hata durumları `SparkToast` ile sunulur.

**JSON formatı (`version: 1`):**
...
2. **Satıcılar:** `findOrCreate` stratejisi (`name` lower-case). Logo URI varsa kopyalanır.
...
5. Tüm girdiler `inputValidation.ts` (sanitize + `stripDangerousKeys`) üzerinden geçirilir (§7.7).
6. **Atomiklik:** `withTransactionAsync` — ilk hata tüm değişiklikleri geri alır, kısmi import yoktur.

**Export akışı:** `buildBackupPayload` → `JSON.stringify` → `expo-file-system`’in `new File(Paths.cache, ...)` + `file.write(...)` API’si → `Sharing.shareAsync` (MIME `application/json`). Dosya adı: `spark-backup_<start>_<end>.json`.

**Versiyonlama:** `BACKUP_FORMAT_VERSION` sabit; import, `payload.version > BACKUP_FORMAT_VERSION` olduğunda `UNSUPPORTED_VERSION` fırlatır → kullanıcıya “uygulamayı güncelleyin” uyarısı.
...
- **Kurallar:** bütçe %80/%100/aşım, kategori limiti yaklaşma/aşım, hedef riski, fiş taslağı bekliyor, API key yok, tarama/ağ hatası, yeni ay bütçe yok.
- **Sessize alma:** tür bazlı (bütçe / kategori / hedef / fiş / sistem).
...
### 8.1 Tamamlanan performans iyileştirmeleri (P1–P12)
