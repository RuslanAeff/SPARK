...
- **Tema senkronu:** `useAppTheme()` (merkezî `themeStore`, §6.1.2) — uygulama teması React store üzerinden değişir; native Android Activity teması değiştirilmez. Statik `Colors` proxy'sinin `StyleSheet` içinde donmaması için stiller şemaya göre yeniden üretilir.
...
- **Görsel hiyerarşi:** tek satırlı 56 dp header; 40 dp görünen/48 dp etkin navigasyon hedefleri. Filtreler 36 dp, gölgesiz tonal çiplerdir. Bildirim kartı 36 dp tonal ikon + 18 dp glyph kullanır; `info / warning / critical` önemleri sırasıyla yeşil / amber / kırmızı accent ile ayrılır. Dekoratif bilgi veya kart-içi `close` ikonu yoktur; kartın ana yüzeyi detayı açar. Okunmamış durum büyük dolu yüzey yerine ince sol rail + düşük alfa kart tonu ile gösterilir.
- **Tekli silme:** kart sola kaydırıldığında sağda 88 dp kırmızı `Sil` yüzeyi (`trash-can-outline` + metin) açılır. Jest yalnız aksiyonu gösterir; veri, kullanıcı bu açık hedefe dokununca silinir. Böylece yatay gezinme/kazara sürükleme doğrudan yıkıcı işlem üretmez. Aynı anda yalnız bir satır açık kalır; filtre değişimi, liste kaydırma, seçim modu ve Android geri tuşu açık satırı kapatır. Gesture kökü `app/_layout.tsx` içindeki `GestureHandlerRootView`'dır.
- **Çoklu seçim:** karta 380 ms uzun basma seçim modunu açar ve ilk kartı seçer; sonraki normal dokunuşlar yalnız checkbox seçimini değiştirir, detay açmaz/okundu durumuna dokunmaz. Header `İptal / {count} bildirim seçildi / Sil` düzenine geçer; silme öncesinde ortak `GlassDeleteModal` ile adet içeren onay gösterilir. Seçim modunda kart swipe'ı, filtre çipleri ve pull-to-refresh kapalıdır; bu iki silme modelinin gesture/state çakışmasını önler.
- **Kalıcılık ve yarış güvenliği:** `dismissMany(ids)` mevcut kimlikleri tek SQLite transaction'ında bir kez süzer ve feed'i tek yazmada günceller. `sys-no-api-key` / `sys-scan-err` kalıcı dismissal bayrakları aynı transaction'da commit edilir. Sync, okundu işaretleme, sessize alma, fiş bildirimi ekleme ve silme aynı hata-dayanıklı mutation kuyruğundan geçer; eski JSON snapshot'ının yeni sonucu geri yazması engellenir. Mutasyonlar toleranslı `[] / {}` fallback loader'larını kullanmaz; DB/JSON okuma hatasında yazmadan durur. Sync'in feed + “üretildi” rules snapshot'ı da tek transaction'dır; yarım commit bildirimi sonsuza dek bastıramaz.
- **Aksiyon/erişilebilirlik kuralları:** “tümünü okundu yap” yalnız `unreadCount > 0` iken header'da kompakt `check-all` olarak görünür. Filtreler `accessibilityRole="tab"` + `selected` state; normal kartlar `button` + uzun-bas hint'i, seçim kartları `checkbox` + `checked` state taşır. Kaydırma sonucu açılan `Sil` de ayrı erişilebilir düğmedir. Kartlara mount/stagger animasyonu eklenmez (P20/P15 flicker koruması).
...
- `src/components/ThemeScheduler.tsx` kökte mount edilir; **60 s aralıkta** `src/utils/themeSchedule.ts` üzerinden DB’deki tema ayarlarını okur ve sonucu `setAppThemeScheme()` ile yalnız React tema store’una uygular. Native `uiMode`/Activity değiştirilmez.
- `loadThemeSettings` otomatik ve manuel tercihleri **tek sorguda** okur. Uygulama işlemleri generation token ile **latest-wins** çalışır; geç tamamlanan eski bir zamanlayıcı sorgusu kullanıcının yeni tema seçimini geri alamaz.
- `setAutoThemeSchedule` / `setManualTheme` ayarlar ekranı tarafından kullanılır; **otomatik tema** gün doğumu/batımı (veya kullanıcı saatleri) bazında geçiş yapar.
- `AutoThemeScheduleToggle` ve `ThemeScheduler` bileşenleri tüm tema tercih UI’sını yönetir.
- **İlk açılış senkronu:** `useDatabase` hook'u `isReady=true` vermeden **önce** `applyThemeFromDatabase()`'i `await` eder. Dil, para birimi ve onboarding hedefi de `app/_layout.tsx` içindeki opak başlangıç perdesi açılmadan önce çözülür; kullanıcı yanlış temalı/yerelleştirilmiş ara kare görmez.

### 6.1.2 Merkezi tema mağazası (`src/theme/themeStore.ts`) — **single source of truth**

#### Sebep-sonuç zinciri (çözülen regresyon, P12/P20)

Eski akışta kayıtlı tema async DB okumasından sonra native `Appearance.setColorScheme()` ile uygulanıyordu. Android'de bu çağrı `uiMode` değişimi/Activity yeniden oluşturması üretebildiği için tam ekran ışık patlaması oluşabiliyor; ilk DB cevabından önce üretilen `StyleSheet` değerleri de yanlış OS şemasında donabiliyordu. Native splash erken kapanınca tema, dil, para birimi ve hedef rota değişimleri ayrı karelerde görünür hale geliyordu.

#### Çözüm mimarisi

`themeStore`, `useSyncExternalStore` tabanlı uygulama içi tema kaynağıdır:

1. Kayıtlı tercih yüklenene kadar **OS kanalı** (`Appearance.addChangeListener`) sistem şemasını izler.
2. DB tercihi uygulandığında `setAppThemeScheme()` store değerini günceller ve tüm `useAppTheme()` abonelerini senkron yeniden render eder.
3. Manuel/zamanlanmış geçişler aynı React kanalını kullanır; native `Appearance.setColorScheme()` çağrılmaz, dolayısıyla Android Activity yeniden oluşturulmaz.
4. Başlangıçta native splash ve JS `BootSurface` aynı `#050505` yüzeyi ve aynı görseli kullanır. Tüm başlangıç bağımlılıkları hazır olduktan sonra yalnız bir adet 300 ms fade ile uygulama açılır.
...
- `src/utils/themeSchedule.ts` — DB okuması tek sorgudur; her uygulama generation token ile korunur. Ayar yazan yollar bekleyen eski sorguları geçersiz kılmalı ve `setAppThemeScheme()` kullanmalıdır.
- `src/hooks/useDatabase.ts` — `initializeDatabase()` sonrasında `await applyThemeFromDatabase()` çalışır, ancak sonra `setIsReady(true)` gelir. Böylece tema başlangıç perdesi açılmadan önce store'a uygulanır.
- `app/_layout.tsx` — native splash, DB/tema/dil/para birimi/onboarding hedefi ve root layout hazır olana kadar tutulur. Stack perde altında mount edilebilir; yanlış onboarding rotası `animation: 'none'` ile görünmeden düzeltilir. Hazırlık tamamlanınca native splash kapatılır ve JS perdesi iki frame sonrasında tek fade ile kaldırılır.
...
| P12 | **Aydınlık modda kartların içi siyah + tema değişiminde tam-ekran flash** | Uygulama içi tema için `Appearance.setColorScheme()` kullanmak Android `uiMode`/Activity yeniden oluşturmasını tetikleyebiliyor; ayrıca ilk DB okumasından önce üretilen StyleSheet yanlış şemada donuyordu. **Çözüm:** `themeStore` artık uygulama temasını yalnız React `useSyncExternalStore` kanalında yönetir (`setAppThemeScheme`); native Activity teması değiştirilmez. Kayıtlı tema `isReady=true` öncesinde uygulanır, tüm tema-bağımlı bileşenler `useAppTheme()` + `useMemo([scheme])` kullanır. Tema DB okuması tek sorgudur ve generation token ile **latest-wins** çalışır; geç tamamlanan scheduler sorgusu yeni kullanıcı seçimini geri çeviremez. | `src/theme/themeStore.ts`, `src/utils/themeSchedule.ts`, `src/hooks/useDatabase.ts`, tema-bağımlı ekran/bileşenler |
...
| P15 | **`SparkToast` / işlem bildirimi flicker'ı** | Eski toast her gösterimde yeni RN `Modal` (Android native pencere) açıyor, başarı durumunda tüm ekranı %60 karartıyor ve replacement sırasında eski callback yeni toast'ı silebiliyordu. **Çözüm:** kalıcı React overlay host; native `Modal` ve tam-ekran dim yok. Callback'ler generation+`finished` korumalı, progress native-driver `scaleX`, aynı yerleşim türündeki yeni içerik görünür yüzey üzerinde güncellenir. Açık native sheet/dialog içine yerel host yansıtılır; modal kapanınca root kopyası kesintisiz sürer. | `src/components/SparkToast.tsx`, `BottomSheetModal.tsx`, `GlassDeleteModal.tsx`, `ConfirmModal.tsx` |
...
| P18 | **`Colors` proxy eski şemayı okuyabiliyor (P12 nüksü)** | `Colors` proxy doğrudan `themeStore.getAppThemeSnapshot()` okur; `Appearance` yalnız store ilk yüklenirken fallback'tir. Böylece manuel/zamanlanmış tema, native şemadan bağımsız olarak yeni StyleSheet üretimlerinde de aynı kalır. | `src/theme/colors.ts`, `src/theme/themeStore.ts` |
...
| P20 | **Soğuk açılışta siyah/beyaz/tema yüzeylerinin art arda parlaması** | Native splash erken kapanıyor; tema, dil, para birimi ve onboarding rotası farklı async karelerde çözülüyordu. Dil provider'ı bir kare `null` döndürüyor, native pencere görünüyordu. **Çözüm:** `expo-splash-screen` native splash'i tutar; day/night/native window + JS BootSurface tek `#050505` ve aynı 160px görseldir. DB/tema/dil/para birimi/hedef rota hazır ve layout commit olmadan reveal başlamaz; yanlış onboarding rotası opak perde altında `animation:none` ile düzeltilir, ardından tek 300ms fade yapılır. Provider çocukları hiçbir zaman `null` değildir, ErrorBoundary hata halinde splash'i kapatır, gizli tablar cold-start'ta lazy mount edilir. | `app/_layout.tsx`, `app.json`, `LanguageContext.tsx`, `CurrencyContext.tsx`, `useOnboardingStatus.ts`, `app/(tabs)/_layout.tsx` |
...
| P22 | **120Hz cihazlarda scroll sırasında FPS/Hz düşüşü / texture flash riski** | Android sınır glow'u ana ScrollView'larda kapalıdır. Kart giriş animasyonu kısaltıldı; `renderToHardwareTextureAndroid` zorlaması kaldırıldı çünkü veri yenilemede texture yeniden ayırımı bazı GPU/driver'larda tek-kare siyah/boş yüzey üretebiliyordu. | `src/components/AnimatedCard.tsx`, `app/(tabs)/index.tsx`, `app/(tabs)/analytics.tsx` |
...
12. Tema-bağımlı her component **`useColorScheme()` değil** `useAppTheme()` kullanır (P12, §6.1.2). `Colors` proxy'sinin bayat `StyleSheet`'te donmaması için `scheme` değişkeni `useMemo([scheme])` deps'ine verilir. Uygulama içi tema için **`Appearance.setColorScheme()` yasaktır**; `setAppThemeScheme()` kullanılmalıdır (Android Activity recreation/flicker guard'ı).
13. DB'den okunan kullanıcı tercihleri UI'ı etkiliyorsa (tema, dil, para birimi) başlangıç perdesi açılmadan **önce** hazır olmalıdır. Tema `useDatabase` içinde uygulanır; provider readiness durumları `app/_layout.tsx` tarafından aynı reveal kapısında beklenir (P12/P20).
...
| **Startup** | Native splash → React içerik sürekliliği | ✅ **Tamamlandı (Temmuz 2026 — P20):** native splash manuel tutulur; DB/tema/dil/para birimi/onboarding rotası tek gate; sabit `#050505` BootSurface; aynı görsel ölçüsü; iki commit frame'i ardından tek fade. İlk-run cihaz dili de perde kalkmadan çözülür. |
...
3. Çok sık değişen (ör. `refreshKey`) ile stabil action/listener kanalını **ayrı context’e** böl — `RefreshKeyContext` ve `RefreshActionsContext` bu ayrımı uygular; gizli sekmeler invalidation'da render olmaz.
...
- **Otomatik test (kuruldu — v2.3; Temmuz 2026 genişletildi):** **Jest** (`jest-expo` preset) ile birim test altyapısı; `npm test`, `npm run test:watch`, `npm run typecheck` script'leri. **201 test, 30 suite** — saf-mantık: `formatCurrency`, `dateUtils`, `itemNameNormalizer`, `inputValidation`, `budgetCycle`, `currencyMeta`, `vendorPlaceholders`, `receiptLineDiscountUi`, `debtMath`, `spendingProjection` (utils) + `receiptLineMerge`, `geminiParse` (coerce/items-cap/JSON onarım), `subscriptionDetector` (detectForVendor) (services) + `localeParity` (i18n 4-dil anahtar paritesi — ratchet, aşağı bkz.); **hook/state regresyonları:** `useBudget` latest-wins, tema DB uygulamasında latest-wins, startup sırasında LanguageProvider'ın yüzeyi koruması, odaklı refresh/inaktif sekme render guard'ı ve eşzamanlı notification mute birleştirmesi; **bileşen/ekran testleri:** analiz kartları, `SparkToast` (kalıcı overlay, replacement, modal-local host) ve bildirim merkezi (mark-all görünürlüğü, detay/kaydırmalı-silme ayrımı, kanal filtreleri, uzun-bas seçim modu ve tek çağrılı toplu silme); ayrı swipe-card testi 88 dp aksiyon genişliğini, seçim sırasında erişilebilirlik/touch kilidini ve açık-satır ref temizliğini korur. **Bildirim depolama testleri:** çoklu ID'yi tek feed yazısında kaldırma, system dismissal bayraklarının atomik commit'i, feed+rules rollback, strict-read/write guard'ı ve hata sonrası devam eden mutation kuyruğu. **Saf test deseni:** native bağımlı modülleri (`secureKeyStore`/`expo-secure-store`, `db/database`/`expo-sqlite`, `db/*Dao`) `jest.mock(...)` ile değiştir. **Bileşen test altyapısı:** `@testing-library/react-native@14` + `test-renderer@1`; RNTL 14'te `render` **async**'tir. **GitHub Actions CI** (`.github/workflows/ci.yml`): `npm ci` → `npm run typecheck` → `npm test --ci`.
...
- Uygulama içi tema için `Appearance.setColorScheme(...)` **eklenmemelidir**; hızlı eylem, widget veya bildirim yolu dahil tüm uygulama teması değişimleri `setAppThemeScheme()` üzerinden gitmelidir. Native çağrı Android Activity recreation/flicker regresyonunu (P12/P20) geri getirebilir.
- DB'den okunan kullanıcı tercihleri ilk görünür kareyi etkiliyorsa (tema, dil, para birimi, onboarding hedefi) `app/_layout.tsx` başlangıç reveal kapısına dahil edilmelidir (§6.1.1, P20).
