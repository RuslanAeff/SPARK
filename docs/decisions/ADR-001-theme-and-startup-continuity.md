...
4. Native splash otomatik kapanmaz. DB init/seed, kayıtlı tema, dil sözlüğü, para birimi, onboarding hedef rotası ve root layout hazır olana kadar native splash ile aynı `#050505` JS `BootSurface` görünür.
5. Başlangıç rotası opak perde altında animasyonsuz düzeltilir; iki commit frame'i sonrasında tek reveal fade yapılır.
6. Provider'lar yüklenirken çocukları `null` yapmaz. Hazır olma sinyalleri root reveal gate'inde toplanır.
7. Transient başarı/bilgi geri bildirimi yeni native Modal açmak yerine kalıcı React overlay host üzerinde gösterilir; native sheet içindeyken modal-local host devamlılığı korunur.
...
- Otomatik: tema latest-wins, LanguageProvider yüzey sürekliliği, SparkToast replacement/modal-host testleri ve `npm run typecheck`.
- Native: cold start, light/dark kayıtlı tercih, ilk kurulum/onboarding rotası, Samsung/Android modal ve gerçek APK üzerinde görsel kontrol. Jest fiziksel ilk kareyi tek başına doğrulamaz.
