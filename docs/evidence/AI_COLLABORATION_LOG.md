...
| `AI-2026-08-25-SETTINGS-FLAT-HIERARCHY-001` | 2026-08-25 | Ayarlar ana menüsü ile dört alt sayfayı kalıcı kart yığınından modern, düz ve görev odaklı bir bilgi mimarisine geçirmek | Kullanıcı önce Genel, Bütçe ve planlama, Veri ve yedek ve Yapay zekâ içeriğinin kartlardan çıkarılmasını istedi; ardından ana Ayarlar ekranındaki aynı dört grup kartını da kaldırma önerisini değerlendirip uygulamayı açıkça onayladı. Commit, push veya yayın istemedi | `analiz`, `kod`, `test`, `inceleme`, `dokümantasyon` | Evet | Ana menü ve alt sayfalarda ortak kart-dışı bölüm/gezinme satırı; renkli grup kimliği ve tam satır dokunma alanı; sarmalanan açıklama; ince ayırıcı; yalnız gerçek kontrollerde sınır; düz bütçe geçmişi; kenar vurgulu yedek durumu | Ana menü genişletmesi uygulandı; typecheck, ana menü+ortak satır odaklı 2/2 suite ve 4/4 test, tam 112/112 suite ve 912/912 test geçti; fiziksel Android optik kabulü bekleniyor |
| `AI-2026-08-25-LIVING-WORDMARK-001` | 2026-08-25 | Dashboard ve Ayarlar'daki SPARK metnini güçlü, ortak, ölçülü ve dokunmaya tepkili yaşayan AI kimliğine dönüştürmek | Kullanıcı mevcut wordmark'ı zayıf buldu; seçili vurgu renginde Jarvis/Friday hissi veren hareketli bir imza istedi, “SPARK Yaşayan Çekirdek İmzası” adını seçti ve güvenli tag/deney branch'i oluşturduktan sonra uygulamayı açıkça başlattı. Art arda cihaz geri bildirimleriyle aralık/sol hiza/çok-yönlü hareket, dokunma tepkisi, optik nokta konumları, beyaz blokların kaldırılması, aydınlık uç kapsamı ve tok/sıkı geometriyi belirledi. Altıncı prototipte büyütmeyi onayladı fakat dokunma olmadan ortam hareketinin hâlâ kendini yeterince göstermediğini, sağ-sol akış ve dalgalanmanın artırılmasını istedi. Commit, push, merge, ikon veya splash değişikliği istemedi | `analiz`, `tasarım`, `kod`, `test`, `inceleme`, `dokümantasyon` | Evet | Tok/sıkı sabit layout; saf vurgu tonlu organik çekirdekler; üç UI-thread saatinden iki karşıt akış+iki fazlı dalga+iki sis katmanı; dokunmada A'dan çift akış; tema ve yaşam döngüsü kapıları | Yedinci prototip uygulandı; typecheck, odaklı 7/7 ve tam 909/909 test geçti; fiziksel Android dokunmasız hareket/pil kabulü bekleniyor |
| `AI-2026-08-25-VENDOR-BULK-DELETE-001` | 2026-08-25 | Veri ve yedek içindeki Satıcı Yönetimi'ne güvenli çoklu seçim ve toplu silme eklemek | Kullanıcı satıcıların yalnız tek tek seçilip silinebildiğini belirtti ve toplu seçimle silme özelliğini açıkça istedi. Commit, push veya yayın istemedi | `analiz`, `kod`, `test`, `inceleme`, `dokümantasyon` | Evet | Açık seçim modu; seçili sayaç; tümünü seç/temizle/iptal; erişilebilir checkbox durumu; toplu bağlı işlem sayımı; geri alınamazlık onayı; tek atomik DAO silme sınırı; dört dil | Uygulandı; typecheck, odaklı 19/19 ve tam 919/919 test geçti; `git diff --check` temiz; fiziksel Android kabulü bekleniyor |
| `AI-2026-08-25-ANDROID-NOTIFICATION-WAKE-HARDENING-001` | 2026-08-25 | Vakti bilinen uyarıları uygulama açılmadan Android paneline ulaştırmak ve uygulama açılışında üretilen gecikmiş uyarının zamanında teslim edilmiş gibi görünmesini engellemek | Kullanıcı gerçek Android panelinde, uygulamaya girdikten sonra saat `03:47`'de oluşan gecikmiş Multisport uyarısını gösterdi; önceki düzeltmeye rağmen kapalı uygulama davranışının kapsamlı biçimde yeniden incelenip çözülmesini istedi. Commit, push, APK build veya yayın istemedi | `analiz`, `kod`, `test`, `inceleme`, `dokümantasyon` | Evet | App-open catch-up/feed ile gerçek future native alarmın ayrılması; native envanter read-back+retry+rollback; kanal/plan sağlığı ve onarım; mutasyon sonrası best-effort senkron; eski hatalı tepsi kopyası temizliği; dört dil; standalone cihaz kabul matrisi | Uygulandı; ikinci ajan oturumunun kapanış incelemesiyle birlikte typecheck, odaklı 148/148 ve tam 943/943 test geçti; public Expo config ve diff kontrolü temiz; standalone Android kabulü bekleniyor |
| `AI-2026-09-03-ONBOARDING-LANGUAGE-FIRST-001` | 2026-09-03 | Uygulamayı ilk kuran kullanıcının, tanıtım içeriğinden önce kendi dilini seçmesini sağlamak | Kullanıcı ilk kurulumda dil seçiminin en başta gelmesi gerektiğini belirtti ve onboarding ekranının düzenlenmesini istedi. Commit, push veya yayın istemedi | `analiz`, `kod`, `test`, `dokümantasyon` | Evet | Sayfa sırasının dil → karşılama → bütçe → tamamlama olarak değiştirilmesi; birincil eylem etiketinin sayfaya göre ayrıştırılması; bütçe doğrulaması ve atlama bağlantısının indeks bağımlılıklarının korunması; onboarding için ilk regresyon testinin yazılması; kullanıcı isteğiyle para birimi seçiciyi AZN, USD, TRY, EUR, PLN sırasına alma ve sırayı tanımlandığı ortak kaynakta teste bağlama | Uygulandı; typecheck, onboarding+para birimi odaklı 12/12 test ve tam 117/117 suite, 967/967 test geçti; `git diff --check` temiz; temiz kurulum cihaz kabulü bekleniyor |
| `AI-2026-09-03-BUDGET-PERIOD-INTEGRITY-001` | 2026-09-03 | Aylık bütçe sistemindeki çakışan dönem, gelecek döneme yazma ve geri dönüp düzeltememe sorunlarını kökten gidermek | Kullanıcı bütçe sistemini "uygulamanın bel kemiği" olarak tanımladı; gerçek cihaz ekranında aynı anda iki "MEVCUT" bütçe kartını gösterdi, gelecek ayların bütçesinin şimdiden ayarlanabilmesini ve yanlış kaydın düzeltilememesini bildirdi. Önce tarama ve plan istedi; ardından üç tasarım kararını (otomatik netleştirme + elle çözüm, silmede harcamaların korunması, son 5 dönem) açıkça onayladı. Commit, push veya yayın istemedi | `analiz`, `plan`, `kod`, `test`, `dokümantasyon` | Evet | Kök nedenin belgelenmiş ama kodda zorlanmayan ADR-008 değişmezi olduğunun tespiti; iki yazma yolunun farklı "dönem" kavramı kullandığının ve temizlik sorgusunun yalnız birebir `period_start` eşleşmesini kapattığının gösterilmesi; tek yazma giriş noktası ve kesişim temizliği; yetkili satır kuralının tüm ekranlarda tekleştirilmesi; saf çakışma modülü; gelecek dönem kilidi; 5 dönemlik düzenleme penceresi ve ihlal kaydı için istisna; onay pencereli silme; onboarding'in gerçek döngü gününü yazması; dört dil | Uygulandı; typecheck, odaklı 20/20 test ve tam 116/116 suite, 961/961 test geçti; `git diff --check` temiz; gerçek SQLite ve bozuk verinin cihazda düzelmesi doğrulaması bekleniyor |
| `AI-2026-09-03-PRICE-PAGE-JUMP-001` | 2026-09-03 | Fiyat takibi kartındaki çok sayfalı listede uzak bir sayfaya tek dokunuşla geçebilmek | Kullanıcı gerçek ekran görüntüsü üzerine not düşerek sayfa noktalarının yerine “hızlı rakam seçimi ile geçiş” istedi ve tasarımın karta oturmasına dikkat edilmesini vurguladı. Commit, push veya yayın istemedi | `analiz`, `kod`, `test`, `dokümantasyon` | Evet | Pasif nokta göstergesinin dokunulabilir rakam şeridine dönüştürülmesi; pager ref'i üzerinden doğrudan sayfa geçişi; programatik kaydırmada göstergenin elle eşitlenmesi; sabit pill genişliğiyle hedef kaymasının önlenmesi; aktif rakamın görünür alana çekilmesi; liste kısalınca göstergenin sıkıştırılması; dört dil erişilebilirlik etiketi | Uygulandı; typecheck, odaklı 4/4 test ve tam 115/115 suite, 947/947 test geçti; `git diff --check` temiz; fiziksel Android dokunma/jest kabulü bekleniyor |
| `AI-2026-09-03-PLAY-RELEASE-COUNCIL-001` | 2026-09-02 – 2026-09-03 | Google Play üretim yayınına hazırlık durumunu tek modelin denetimine bırakmadan, iki farklı model üzerinden çapraz doğrulayarak tek uygulanabilir plana indirmek | Kullanıcı aynı görevi iki modele ayrı ayrı verdi, taraflara birbirinin raporunu iletti ve “konsey” olarak tek planda uzlaşmalarını istedi. Denetim turlarında hiçbir dosyanın değiştirilmemesini açıkça yasakladı; commit, push, build veya yayın istemedi | `analiz`, `inceleme`, `dokümantasyon` | Evet (iki model) | Bağımsız salt-okunur depo denetimi; karşı modelin iddialarının depoda çalıştırılan komutlarla doğrulanması; dört çelişkinin kapatılması; doğrulanamayan iddianın sınırlama olarak işaretlenmesi; 12 aşamalı ortak yayın planı | Plan üretildi ve iki taraf mutabık kaldı; hiçbir dosya değişmedi, çalışma ağacı denetim boyunca temiz kaldı. Uygulama Aşama 1'deki dört insan kararına bağlandı |
| `AI-2026-09-02-PRIVACY-POLICY-ACCURACY-001` | 2026-09-02 | Yayınlanmış gizlilik politikasını uygulamanın gerçek veri toplama ve dışa aktarma davranışıyla yeniden hizalamak | Kullanıcı v2.3.0'dan bu yana eklenen özelliklerden sonra politikada değişmesi gereken bir şey olup olmadığını sordu; bulguları onaylayıp düzeltilmesini istedi. Commit, push veya yayınlama istemedi | `analiz`, `inceleme`, `dokümantasyon` | Evet | Kod denetimiyle politikadaki yanlış "fiş tarama dışında hiçbir veri iletilmez" iddiasının tespiti: `suggestProductMatch` ürün kimlik alanlarını ikinci bir Gemini isteğiyle gönderiyor. Eksik veri kategorilerinin (borç, ödeme planı, fiş kalemi, ürün kimliği, ek gelir, bildirim geçmişi) eklenmesi; bildirim içeriğinin OS deposunda bulunması ve kilit ekranı gizliliği; yedek dışa aktarımının hedef servis sorumluluğu; sürüm damgası | Uygulandı; TR/EN bölümleri simetrik, ağ çağrıları ve `app.json` izinleri koddan doğrulandı. Politikanın yayınlandığı ayrı GitHub Pages deposuna kopyalanması kullanıcıya bırakıldı |
| `AI-2026-09-02-DONUT-ARROW-NAV-001` | 2026-09-02 | Analiz sekmesindeki Davranışsal Analiz kartındaki küçük donut dilimlerini dokunmadan erişilebilir kılmak | Kullanıcı ihtiyaç/keyif ve hafta içi/hafta sonu donut'larındaki küçük dilimlerin dokunarak seçilmesinin zor olduğunu belirtti; Dashboard bütçe donutundaki önceki/sonraki ok kontrolünün aynı karta da uygulanmasını istedi. Commit/push istemedi | `analiz`, `kod`, `test`, `dokümantasyon` | Evet | `DashboardBudgetDonut`'taki döngüsel önceki/sonraki gezinme deseninin `DonutCard`'ın iki bağımsız donut'una (ihtiyaç/keyif, hafta içi/hafta sonu) taşınması; seçim yokken mevcut kaydırma ipucunun korunması; dört dil; odaklı regresyon testleri | Uygulandı; typecheck ve tam 115/115 suite, 945/945 test geçti; fiziksel Android dokunma/erişilebilirlik kabulü bekleniyor |
| `AI-2026-09-05-PERSONAL-INFLATION-CARD-001` | 2026-09-05 | Analiz sayfasındaki gereksiz kartların ayıklanması ve farklılaştırıcı yeni bir kart fikri | Kullanıcı önce hangi kartların gerçekten gereksiz olduğunu sordu ve "çok fazla gereksiz olmadıkça çıkarmayalım" sınırını koydu; ardından kritik gerekliliğe dayalı yeni bir kart fikri istedi. Öneri kabul edilince karta profesyonel bir ad verilmesini ve tespit edilen ölü kodun silinmesini istedi. Commit ve push kullanıcıya bırakıldı | `analiz`, `kod`, `test`, `dokümantasyon` | Evet | 14 kartın işlevsel çakışma taraması: yalnız `heatmap`↔`chart` çakışması bulundu, kart zaten varsayılanda kapalı olduğu ve takvim hizası farklı bir okuma sunduğu için silme önerilmedi; UX'te kart çıkarılmadı. Referanssız `analytics/BudgetCard.tsx` ve `analytics/TimeOfDayCard.tsx` ile bunlara bağlı tip, 18 stil bloğu ve 16 çeviri anahtarı dört dilden kaldırıldı. Yeni kart: Laspeyres fiyat endeksiyle harcama değişiminin fiyat ve sepet etkisine ayrıştırılması; birim fiyatta medyan; karşılaştırılabilir tamamlanmış-gün pencereleri; sepet kapsamının kartta açıkça bildirilmesi | Uygulandı; kullanıcı ekran görüntüsü üzerinden kartın kapsam metnindeki yanlış paydayı ve tasarım tekrarını sordurdu, ardından indirimli fiyatların endeksi nasıl etkilediğini sordu ve üçlü ayrışma seçeneğini seçti; kullanıcı kapsam cümlesindeki "kalem" teriminin ne olduğunu sordu ve uzun cümlenin kartı çirkinleştirdiğini, DonutCard'daki (i) butonu deseniyle taşınabileceğini önerdi; kapsam notu tek satıra indirildi, uzun açıklama (i) modalına taşındı; typecheck exit 0; enflasyon hesabı 14/14, kart 9/9 test; tam Jest 120/120 suite ve 998/998 test geçti; fiziksel Android kabulü ve gerçek fiş verisiyle doğrulama bekleniyor |
| `AI-2026-09-06-DATA-RESET-001` | 2026-09-06 | Verileri sıfırlama özelliğinin nereye konacağı ve elit/profesyonel görünecek tasarımı | Kullanıcı demo veri setiyle uygulamayı sınadıktan sonra sıfırlama ihtiyacını bildirdi; yer ve tasarım kararını sordu. Commit ve push kullanıcıya bırakıldı | `analiz`, `kod`, `test`, `dokümantasyon` | Evet | Konum olarak Veri ve Yedek sayfasının EN ALTI seçildi (kaçış yolu olan yedek bölümü hemen üstte kalsın, kullanıcı oraya inmek için her şeyin yanından geçsin). Görsel dil kasıtlı olarak sakin tutuldu: dolu kırmızı panel yerine ince danger kenarlıklı kart ve hayalet düğme; asıl koruma renkte değil onay kapısında. Onay penceresi hâle desenini danger tonunda kullanır ve üç katmanlıdır: gerçek sayımlar, korunanların listesi ve basılı tutarak onay. Kullanıcı ilk turda hem "Tehlikeli Bölge" başlığını hem de onay kelimesini yazdırmayı reddetti ("kullananlar çocuk değil", "fazla zorlama"); başlık eylemin adıyla değiştirildi, yazma kapısı yerine 1,4 saniyelik basılı tutma kondu — kazara tetiklenmeye karşı koruma korunurken sürtünme kaldırıldı. Silme kapsamı dar tutuldu; `settings` tablosu ve native alarm defteri bilinçli olarak korundu | Uygulandı; typecheck exit 0; servis 6/6, pencere 7/7 test; tam Jest 122/122 suite ve 1011/1011 test geçti; fiziksel Android kabulü (alarm iptali ve boş duruma dönüş) bekleniyor |
...
### `AI-2026-08-25-LIVING-WORDMARK-001`

| Alan | Kayıt |
|---|---|
| Tarih / saat dilimi | 2026-08-25 / Europe/Warsaw |
| İnsan hedefi ve onayı | Kullanıcı Dashboard sol üstü ve Ayarlar altındaki SPARK adını daha iri, kalın ve yaşayan AI hissi veren ortak bir kimliğe dönüştürmek istedi. Önerilen “SPARK Yaşayan Çekirdek İmzası” adını seçti; kararlı `main` için `spark-before-living-wordmark-v1` etiketini ve `experiment/living-spark-wordmark` dalını uzak depoya kendisi oluşturup gönderdi, ardından uygulamayı açıkça başlattı. Commit, push, merge, app icon veya splash değişikliği istemedi |
| Başlangıç çalışma ağacı | Temiz `experiment/living-spark-wordmark` dalı; `main` ve başlangıç etiketi ayrı kurtarma noktaları olarak korundu |
| İnsan katkısı | İmzanın uygulama içi yazı ve logo rolünü birlikte taşımasını, seçili vurgu renginde akvaryum/AI çekirdeği gibi canlı his vermesini, gerekirse tasarımın sıfırdan denenebilmesini ve nihai görsel kabulün kendisinde kalmasını belirledi. Cihaz geri bildirimleriyle eşit/optik aralık, Dashboard sol hizası, çok-yönlü sis, dokunma tepkisi, yalnız ilk iki noktanın sola alınması, beyaz blokların kaldırılması, aydınlıkta `S`–`K` yaşam kapsamı ve tok/sıkı geometriyi aşamalı seçti. Altıncı prototipte büyütmenin beklediği sonucu verdiğini onayladı; dokunmadan beklerken ortam hareketinin hâlâ az göründüğünü belirterek sağdan-sola, soldan-sağa ve dalgalanan sürekli hareketi artırmayı yedinci prototip gereksinimi yaptı |
| AI katkısı | Mevcut Dashboard/Ayarlar, atomik tema snapshot'ı, SVG ve Reanimated altyapısından ortak imzayı kurup her cihaz geri bildirimini aynı bileşende izole etti. Harfleri ayrı eşit merkezlere, noktaları optik konumlara taşıdı; filtresiz üç tonlu radial-gradient elipsler, A merkezli dokunma tepkisi, aydınlık/karanlık saf hareket profili ve tok/sıkı hero-kompakt layout geliştirdi. Yedinci prototipte yeni shared value veya sürekli zamanlayıcı eklemeden mevcut üç UI-thread saatini hızlandırdı: yatay akış `8,2→6,2`, merkez dalgası `4,6→3,8`, sis `7,1→5,4` saniye. Tek aralıklı çekirdeği soldan-sağa ve sağdan-sola sürekli akan iki çekirdeğe; tek merkez dalgasını yarım fazlı iki dalgaya dönüştürdü. Karanlık sis yollarını da canvas'ın %20/%23'üne genişletip iki sisle birlikte görünür ortam primitive sayısını dörtten altıya çıkardı. Odak/AppState/hareket azaltma kapıları, üç döngü sınırı ve 980 ms dokunma tepkisi korundu; katman varlığı ve süreler teste bağlandı |
| AI önerisi ile insan kararı ayrımı | İnsan yaşayan AI yönünü, deneysel branch güvenliğini, tok/sıkı görünümü, dokunma tepkisini, optik nokta kararlarını, çok tonlu organik dili, tema davranışını ve dokunmasız hareketin daha görünür olmasını seçti. Kesin geometri, optik kaydırmalar, altı primitive'i üç UI-thread saatinden üretme, `6,2/3,8/5,4` saniye ritmi, opaklık/hareket aralıkları, 980 ms tepki ve fallback uygulaması AI tarafından performans ile sakin yaşam dengesi gözetilerek seçildi |
| Değiştirilen alanlar | `src/components/LivingSparkWordmark.tsx`; `app/(tabs)/{index,settings}.tsx`; `src/components/__tests__/LivingSparkWordmark.test.tsx`; `__mocks__/react-native-reanimated.js`; `DESIGN_BRIEF.md`; mimari/geliştirme/kalite ve evidence belgeleri |
| Otomatik doğrulama | İlk–altıncı prototiplerin ara typecheck/odaklı/tam Jest sonuçları korunuyor. Yedinci prototipte typecheck; iki yeni ortam katmanı ve `6,2/3,8/5,4` saniye ritmini kapsayan odaklı 1/1 suite ve 7/7 test; final tam 111/111 suite ve 909/909 test geçti; `git diff --check` temizdi. Paket içindeki mevcut async `act()` ve kontrollü Gemini geliştirme uyarıları ürün sonucunu değiştirmedi |
| Cihaz doğrulaması ve sınır | Önceki Android görüntüleri tipografi, optik noktalar, organik renk dili, dokunma, karanlık görünüm ve aydınlık uç profilini aşamalı şekillendirdi; altıncı prototipin büyütmesi insan tarafından onaylandı. Yedinci prototipte bekleniyor: Dashboard/Ayarlar ve beş vurgu × açık/koyu; dokunmadan en az 20 saniyede iki karşıt yatay akışın, iki kademeli merkez dalgasının ve iki sis çekirdeğinin belirgin fakat okunaklı görünmesi; beyaz blok/sert bant oluşmaması; dokunma tepkisinin ortamdan ayrılması; hızlı sekme/arka plan dönüşünde hareketin durup devam etmesi; hareket azaltma, TalkBack, büyük font ve uzun çalışma pil/ısı. Jest gerçek SVG/Reanimated akıcılığını, optik hareket algısını veya pil etkisini kanıtlamaz |
| Geri dönüş ve yayın sınırı | Beğenilmezse yeniden clone gerekmez: `main` kararlı sürümdür; başlangıç etiketi yeni bir v2 deney dalı için kullanılabilir. Klasik component varyantı yalnız sunum fallback'idir. Bu oturum commit, push, merge, build veya yayın yapmaz |
| Gizlilik | Ekli uygulama ekranı depoya kopyalanmadı; gerçek finansal tutarlar, kişisel dosya yolları veya secret test/doküman içeriğine taşınmadı |

### `AI-2026-08-25-VENDOR-BULK-DELETE-001`

| Alan | Kayıt |
|---|---|
| Tarih / saat dilimi | 2026-08-25 / Europe/Warsaw |
| İnsan hedefi ve onayı | Kullanıcı Veri ve yedek içindeki Satıcı Yönetimi'nde satıcıların yalnız tek tek silinebildiğini belirtti ve toplu seçimle silme özelliğini açıkça istedi. Commit, push veya yayın istemedi |
| Başlangıç çalışma ağacı | Dirty `experiment/living-spark-wordmark`; aynı kullanıcıya ait önceki onaylı ayar/wordmark değişiklikleri korundu ve ilgisiz değişiklikler geri alınmadı |
| İnsan katkısı | Çok satıcılı kullanımda tekli silmenin gereksiz uğraş oluşturduğunu tanımladı ve seçerek toplu silmeyi kabul kriteri yaptı |
| AI katkısı | Mevcut tekli dokunma, uzun basma ve satıcı seçenekleri akışını koruyan açık bir çoklu seçim modu ekledi. Seçili sayacı, tümünü seçme, seçimi temizleme ve iptali görünür kıldı; satıcı kutularını seçim modunda erişilebilir checkbox'a dönüştürdü. Onaydan önce tek parametreli sorguyla seçili satıcıların bağlı işlem toplamını hesapladı. Tekli silmeyi de aynı `deleteMany` sınırına yönlendirip bağlı işlemler ve satıcıları tek `withTransactionAsync` içinde sildi; geçersiz/tekrarlı kimlikleri sorgu öncesinde eledi. TR/EN/AZ/RU metinlerini ve DAO/UI regresyon testlerini ekledi |
| AI önerisi ile insan kararı ayrımı | İnsan toplu seçim ve silme kararını verdi. Ayrı seçim modu, toplu etki özeti, onaydan önce sıfır yazma, tek transaction ve erişilebilirlik ayrıntıları AI tarafından mevcut finansal bütünlük sözleşmesine göre seçildi |
| Değiştirilen alanlar | `app/settings-data.tsx`; `src/db/vendorDao.ts`; `src/components/__tests__/SettingsDataVendorSelection.test.tsx`; `src/db/__tests__/vendorDao.test.ts`; TR/EN kaynakları, AZ/RU map ve üretilmiş locale çıktıları; ürün/mimari/evidence belgeleri |
| Otomatik doğrulama | `npm run typecheck` exit 0; DAO, Satıcı Yönetimi, mevcut ürün eşleştirme ekranı ve locale parity için odaklı 4/4 suite ile 19/19 test geçti. Final `npm test -- --ci --coverage=false --runInBand` ile 114/114 suite ve 919/919 test geçti; `git diff --check` temizdi. Tam pakette ürün sonucunu etkilemeyen mevcut async `act()` ve kontrollü Gemini geliştirme uyarıları görüldü |
| Cihaz doğrulaması ve sınır | Bekleniyor: 0/1/çok/tüm seçim; arama filtresi açıkken görünür ve global seçim; iptal ve onay; bağlı işlemli/işlemsiz metin; silme sonrası satıcı, işlem ve analiz yenilemesi; açık-koyu tema, TR/EN/AZ/RU, büyük font ve TalkBack. Jest gerçek SQLite FK/trigger yaşam döngüsünü, yatay kaydırma ile seçim isabetini ve yıkıcı işlem hissini kanıtlamaz |
| Gizlilik ve yayın sınırı | Testlerde yalnız sentetik satıcı adları, kimlikleri ve işlem sayıları kullanıldı; kullanıcı ekranı veya finansal verisi depoya taşınmadı. Commit, push, build veya nihai insan kabulü yoktur |

### `AI-2026-08-25-ANDROID-NOTIFICATION-WAKE-HARDENING-001`

| Alan | Kayıt |
|---|---|
| Tarih / saat dilimi | 2026-08-25 / Europe/Warsaw |
| İnsan hedefi ve onayı | Kullanıcı Android panelinde uygulamayı açtığı anda oluşan gecikmiş ödeme planı uyarısını gerçek cihaz ekranıyla gösterdi; kullanıcıyı önceden harekete geçirmesi gereken bildirimlerin uygulamaya girilmeden görünmesini, önceki denemede kalan sistem probleminin kökten bulunup çözülmesini açıkça istedi. Commit, push, APK build veya yayın istemedi |
| Başlangıç çalışma ağacı | Dirty `experiment/living-spark-wordmark`; aynı kullanıcıya ait önceki onaylı UI, ayar ve satıcı değişiklikleri korundu, ilgisiz değişiklikler geri alınmadı |
| İnsan katkısı | Teslim zamanını gerçek Android saat damgasıyla ilişkilendirip problemin yalnız metin veya tasarım değil kapalı uygulama teslim semantiği olduğunu belirledi; proaktif uyarının uygulama açılışına bağımlı olmamasını kabul kriteri yaptı |
| AI katkısı | Ekrandaki “Planlanan tarih geçti” mesajının geleceğe yazılmış OS alarmından değil, uygulama mount/resume akışında yeniden hesaplanan feed kaydının anlık Android köprüsünden geldiğini koddan ayırdı. Yalnız gecikmiş ödeme planını değil borç/plan `upcoming-today-overdue`, hedef kontrol noktası ve bütçe eşiklerinin tamamını catch-up ailesi olarak feed-only yaptı. Eski anlık sistem kopyalarını gerçek future handle'ı silmeden temizledi. Future desired-state yazımına native envanter read-back, tek retry, eksik/hata durumunda telafi iptali ve ledger rollback ekledi; doğrulanmayan turda cursor ilerlemesini engelledi. Bildirim ayarlarına doğrulanan/istenen sayı, sonraki alarm, kanal `ready/degraded/blocked/unknown`, hata ve onarım görünümü ekledi; eski async sorgunun yeni panel state'ini ezmesini engelledi. Borç, onaylı plan, hedef, bütçe ve hedef katkısı yazılarından sonra doğrudan best-effort senkron ekledi; senkron hatasının commit olmuş finansal işlemi başarısız göstermesini önledi. Backup ayar/export/restore refresh'ini güçlendirdi ve artık geçerli olmayan backup feed kartını emekliye ayırdı |
| AI önerisi ile insan kararı ayrımı | İnsan kapalı uygulamaya zamanında uyarı ve kök neden düzeltmesi kararını verdi. Catch-up/feed ile future alarm ayrımı, bir kez retry+rollback, health yüzeyi, doğrudan mutasyon senkronu ve eski kopya migrasyonu AI tarafından mevcut ADR-006 ve Expo/Android sınırlarına göre seçildi. `SCHEDULE_EXACT_ALARM` istememe kararı değiştirilmedi; yedek hatırlatması ile çıkarılmış abonelik tahmini dürüstçe uygulama-olayı/feed kapsamında bırakıldı |
| Değiştirilen alanlar | `src/services/{androidNotificationsSetup,reminderScheduler}.ts`; `src/context/NotificationsContext.tsx`; `src/notifications/{reminderNotificationFeed,buildNotifications}.ts`; `app/notifications.tsx`; ilgili borç/plan/hedef/bütçe/yedek/onboarding mutasyon yüzeyleri; TR/EN kaynakları, AZ/RU map ve üretilmiş locale çıktıları; ADR-006, ürün/mimari/geliştirme/kalite ve evidence belgeleri; ilgili Jest testleri |
| Devralan oturum kapanış incelemesi | İlk oturum kullanım limiti nedeniyle son doğrulama turunu tamamlayamadı; ikinci ajan oturumu aynı kapsamı devraldı, bağımsız inceledi ve üç açık nokta kapattı: (1) `androidNotificationsSetup.ts` ledger-rollback dalındaki `Map.values().map(...)` çağrısı ES2025 iterator helper'ıydı ve Hermes'te garanti olmadığı için diziye açıldı; (2) `androidNotificationsSetup.test.ts` içindeki `falls back to immediate delivery after an overdue pending alarm is canceled` testi bu düzeltmenin kaldırdığı eski sözleşmeyi kilitliyordu, mevcut sözleşmeye göre yeniden yazıldı; (3) `DESIGN_BRIEF.md`, `docs/ARCHITECTURE.md` ve ADR-006 Faz 5 bölümündeki “anlık fallback teslimi” ifadeleri koda aykırı kalmıştı, güncel davranışa göre düzeltildi. Kök neden düzeltmesinin kendisi değiştirilmedi |
| Otomatik doğrulama | `npm run typecheck` exit 0. Bildirim scheduler/context/feed/ayar/locale ve hedef katkısı için 9/9 suite, 131/131 test; borç/ödeme planı/hedef/bütçe mutasyon yüzeyleri için 4/4 suite, 17/17 test geçti. Kapanış incelemesi sonrası final `npm test -- --ci --coverage=false` ile 115/115 suite ve 943/943 test geçti; yalnız önceden mevcut async `act()` ile kontrollü Gemini geliştirme logları görüldü. `npx expo config --type public` bildirim eklentisi ve `POST_NOTIFICATIONS` iznini, exact-alarm izninin bulunmadığını doğruladı; `git diff --check` temizdi |
| Cihaz doğrulaması ve sınır | Bekleniyor: standalone development/release APK (Expo Go değil); bildirim izni ve `alerts` kanalı açık; 5–10 dakika geleceğe onaylı plan oluşturma; uygulama sürecini normal kapatma ve ekranı kilitleme/Doze; uygulamayı açmadan panel teslimi; reboot ve APK update sonrası yeniden plan; mute/değiştir/sil iptali; geçmiş tarihli kayıtla uygulama açılışında yeni sistem bildirimi doğmaması; eski catch-up kopyasının temizlenip gerçek `spark:future:v1:` alarmın korunması. Kullanıcı force-stop yaparsa Android teslim garantisi yoktur. Exact-alarm izni istenmediğinden Doze/OEM politikası dakika hassasiyetini geciktirebilir |
| Gizlilik ve yayın sınırı | Ekli ekran görüntüsü ve gerçek satıcı/finans ayrıntıları depoya taşınmadı; testler sentetik kayıtlar kullanır. Secret, API anahtarı veya kişisel dosya yolu belge/test içeriğine eklenmedi. Bu çalışma commit, push, APK build, yayın veya fiziksel cihaz başarısı iddiası yapmaz |

### `AI-2026-09-03-PLAY-RELEASE-COUNCIL-001`

| Alan | Kayıt |
|---|---|
| Tarih / saat dilimi | 2026-09-02 – 2026-09-03 / Europe/Warsaw |
| Oturum biçimi | **Çok modelli konsey.** Bu, günlükteki ilk tek-model olmayan kayıttır. Aynı görev iki ayrı modele bağımsız verildi; taraflar birbirini görmeden denetim yaptı, sonra kullanıcı aracılığıyla rapor ve plan değiş tokuşu yaptı |
| Katılan modeller | Claude Opus 5 (Claude Code) ve GPT-5.6 (Codex CLI). Model kimlikleri kullanıcı beyanına dayanır; bağımsız doğrulanmamıştır |
| İnsan hedefi ve onayı | Kullanıcı uygulamanın “yüzde yüz hazır” olmadığını kabul ederek Play Store hazırlığına başlamak istedi. Her iki modelden de projeyi tarayıp teknik olmayan, sade bir rapor istedi. Denetim turlarında **hiçbir değişiklik yapılmamasını açıkça yasakladı**; commit, push, APK build veya yayın istemedi |
| Repo/başlangıç commit'i | `main` @ `a716505`; gizlilik politikası deposu @ `61a3b9d` |
| Başlangıç çalışma ağacı | Temiz. Denetim ve plan turlarının tamamı boyunca temiz kaldı |
| İnsan katkısı | Konsey yöntemini kurdu ve yürüttü: aynı komutu iki modele verdi, çıktıları karşı tarafa iletti, uzlaşana kadar tur yaptırdı ve nihai planı kabul etti. Yazma yasağını ve kapsamı belirledi. Aşama 1'deki geri dönüşü olmayan kararları kendine ayırdı |
| AI katkısı — ortak | İki model bağımsız olarak aynı beş yayın engelini buldu: production profilinin AAB yerine APK üretmesi; paket adının ilk yayından sonra değiştirilemeyeceği; `READ_EXTERNAL_STORAGE` izninin gereksizliği; Data Safety formunda “hiç veri toplanmıyor” denemeyeceği; hesap şartı varsa 12 kullanıcı × 14 gün kapalı test. Mağaza görsellerinin ve zorunlu Play beyanlarının eksikliği de ortak bulgudur |
| Çelişki 1 — gizlilik politikası | GPT-5.6, güncellenmiş politikada kalan iç çelişkiyi buldu: “tüm veriler yalnızca cihazınızda saklanır” ve “başka üçüncü tarafla paylaşılmaz” ifadeleri, aynı belgedeki Gemini aktarımı bölümüyle çakışıyordu. Claude Opus 5 bu bölümü daha önce kendisi güncellemiş ama iki cümleyi gözden kaçırmıştı; bulguyu dosyada doğrulayıp hatasını kabul etti ve düzeltmeyi plana aldı |
| Çelişki 2 — mikrofon izni | GPT-5.6 çözülmüş Expo yapılandırmasında `RECORD_AUDIO` ve `MODIFY_AUDIO_SETTINGS` izinlerini gördü ve mikrofon izninin AAB'de bulunmaması gerektiğini bildirdi. Claude Opus 5 `npx expo config --type public` çalıştırıp ayrımı netleştirdi: `RECORD_AUDIO` zaten `blockedPermissions` içindedir ve prebuild sırasında manifestten çıkarılır; `MODIFY_AUDIO_SETTINGS` engellenmemiştir ve gerçekten pakete girer. İki taraf, kaynak yapılandırmanın kanıt sayılmayacağı ve kesin kanıtın üretilmiş AAB manifesti olduğu konusunda uzlaştı |
| Çelişki 3 — kademeli yayın | İki modelin de ilk planlarında “ilk sürümü küçük oranlı staged rollout ile yayımla” maddesi vardı. GPT-5.6 son turda bunun yanlış olduğunu, yüzdeli staged rollout'un yalnız güncellemelerde kullanılabildiğini belirtti ve yerine Managed Publishing ile bilinçli ülke seçimini önerdi. Claude Opus 5 düzeltmeyi kabul etti. **Bu hata yalnız çapraz inceleme sayesinde yakalandı; iki plan da bağımsızken aynı yanlışı taşıyordu** |
| Çelişki 4 — bekleyen değişiklikler | Claude Opus 5 plana koşulsuz bir “bekleyen değişiklikleri commit et” adımı koydu. GPT-5.6 çalışma ağacının temiz olduğunu belirtti. Claude Opus 5 `git status --short` ile doğruladı (temiz, commit `a716505`), kendi maddesini geri çekti ve GPT-5.6'nın koşullu formülasyonunu kabul etti |
| Doğrulanamayan iddia | GPT-5.6'nın bildirdiği “30 Eylül 2026 geliştirici doğrulaması ve paket adı kaydı” şartı depodan doğrulanamadı. İddia silinmedi; plana **kullanıcının Play Console'dan bizzat teyit etmesi gereken madde** olarak, Aşama 1'in içine sınırlama etiketiyle alındı |
| Otomatik doğrulama | `npx expo config --type public` çözülmüş Android izin listesini ve `blockedPermissions` içeriğini verdi. `npx expo-doctor` SDK 55 için 20 paketlik yama farkı bildirdi. `npm audit --omit=dev` 36 uyarı gösterdi (1 kritik, 12 yüksek, 22 orta, 1 düşük); etkilenen paketlerin `metro`, `@xmldom/xmldom`, `js-yaml`, `ws`, `browserslist` gibi derleme zinciri bağımlılıkları olduğu ayrıca kontrol edildi. `npm run typecheck` exit 0; `npm test -- --ci --coverage=false` ile 115/115 suite ve 945/945 test geçti. Ağ çağrısı taraması Gemini dışında dış istek bulunmadığını gösterdi |
| Değiştirilen alanlar | Denetim ve plan turlarında **hiçbiri**. Bu kayıt ile `docs/evidence/TRACEABILITY.md` içindeki `SPK-PUB-001` satırı yalnızca sürecin kendisini belgeler |
| Çıktı | 12 aşamalı ortak yayın planı: hesap ve uygulama kimliği · güvenli başlangıç noktası · Expo ve bağımlılık güncellemeleri · production yapılandırması · gerçek AAB izin denetimi · Gemini ve gizlilik şeffaflığı · yedek ve veri güvenliği · mağaza içeriği · Play Console beyanları · internal test ve cihaz testleri · kapalı test · son yayın kapısı ve üretim yayını |
| Yöntemin sınırı | Çapraz doğrulama, iki modelin **aynı** hatayı birlikte gözden kaçırma olasılığını ortadan kaldırmaz; Çelişki 3 bunun gerçekleştiğini ve ancak karşılıklı inceleme turunda düzeldiğini gösterir. Konsey çıktısı bir plandır, yayına uygunluk kanıtı değildir. Modellerin bağımsızlığı yalnız başlangıç turu için geçerlidir; sonraki turlarda taraflar birbirinin metnini gördüğü için yakınsama kısmen etkileşimin ürünüdür |
...
### Çok modelli çapraz doğrulama (konsey) için önerilen metin

Yayın hazırlığı denetiminde kullanılan iki modelli yöntem için
(`AI-2026-09-03-PLAY-RELEASE-COUNCIL-001`, `SPK-PUB-001`):

> Yayın öncesi hazırlık denetiminde tek bir modelin çıktısına dayanmamak için çok
> modelli çapraz doğrulama uygulanmıştır. Aynı denetim görevi iki farklı büyük dil
> modeline birbirinden bağımsız olarak verilmiş, her iki model de depoyu salt-okunur
> biçimde incelemiş ve ayrı rapor üretmiştir. Ardından raporlar ve planlar karşı
> tarafa iletilerek modellerin birbirinin bulgularını değerlendirmesi sağlanmış;
> çelişkiler, iddiaların depoda çalıştırılabilir komutlarla sınanmasıyla
> kapatılmıştır. Doğrulanamayan iddialar silinmemiş, insan tarafından harici olarak
> teyit edilmesi gereken sınırlama şeklinde kayda alınmıştır. Süreç, iki modelin tek
> bir uygulama planında uzlaşmasıyla sonuçlanmıştır.
>
> Yöntemin gözlemlenen katkısı iki yönlüdür. Birincisi, modellerin birbirinin gözden
> kaçırdığı bulguları yakalamasıdır; bu çalışmada bir modelin güncellediği gizlilik
> politikasında bıraktığı iç çelişki, diğer model tarafından tespit edilmiştir.
> İkincisi ve yöntem açısından daha belirleyici olanı, **her iki modelin bağımsız
> raporlarında ortak biçimde bulunan bir olgu hatasının** ancak karşılıklı inceleme
> turunda düzeltilmiş olmasıdır. Bu durum, çapraz doğrulamanın ortak hata riskini
> azalttığını fakat ortadan kaldırmadığını göstermektedir.
>
> Yöntemin sınırları da kayda alınmıştır. Modellerin bağımsızlığı yalnız ilk tur için
> geçerlidir; sonraki turlarda taraflar birbirinin metnini gördüğünden yakınsama
> kısmen etkileşimin ürünüdür. Ayrıca sürecin çıktısı bir uygulama planıdır ve
> yayına uygunluğun kanıtı değildir; teknik doğrulama, üretilmiş uygulama paketi ve
> fiziksel cihaz testleri ayrı kanıt türleri olarak ele alınmıştır. Nihai kabul ve
> geri dönüşü olmayan kararlar insana bırakılmıştır.
