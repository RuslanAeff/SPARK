...
| `SPK-UX-024` | İlk kurulumda karşılama ve tanıtım metni, kullanıcı dilini seçmeden önce gösteriliyordu; kullanıcı anlamadığı bir dilde okumak zorunda kalıyordu | Onboarding sırası dil seçimi → karşılama → bütçe → tamamlama olarak değişir. Dil kartına dokunmak arayüzü anında o dile geçirir. Birincil eylem etiketi sayfaya göre ayrışır: dil sayfasında nötr "ileri", karşılama sayfasında başlangıç daveti. Bütçe doğrulaması ve atlama bağlantısı aynı sayfa indekslerinde kalır. Para birimi seçicisi AZN, USD, TRY, EUR, PLN sırasında sunulur; sıra ortak `DISPLAY_CURRENCIES` kaynağında tanımlıdır ve Ayarlar seçicisiyle tutarlıdır | Mevcut çalışma ağacı; `app/onboarding.tsx`, `src/utils/currencyMeta.ts`, `src/components/__tests__/OnboardingScreen.test.tsx`, `src/utils/__tests__/currencyMeta.test.ts`, `DESIGN_BRIEF.md` | `AI-2026-09-03-ONBOARDING-LANGUAGE-FIRST-001`: `npm run typecheck` exit 0; onboarding+para birimi odaklı 12/12 test; tam Jest 117/117 suite ve 967/967 test geçti; `git diff --check` temiz | Android'de temiz kurulum: ilk açılışta dil ekranı, dört dilde anında geçiş, geri tuşu davranışı, atla bağlantısı, klavye açıkken bütçe adımı ve tamamlama akışı bekleniyor | `E0` | Jest sayfa sırasını ve eylem etiketini doğrular; gerçek yatay sayfalama jesti, ilk açılış performansı ve dil değişiminin anlık görsel etkisi cihaz kabulü gerektirir |
| `SPK-DOM-005` | Bir takvim gününü iki aktif bütçe dönemi kapsayabiliyordu: aynı harcama iki bütçede sayılıyor, geçmiş şeridinde iki "mevcut" rozeti çıkıyor ve ekranlar birbiriyle çelişiyordu. Ayrıca başlamamış döneme bütçe yazılabiliyor, yanlış oluşan kayıt hiçbir yoldan düzeltilemiyordu | ADR-008 değişmezi koda taşınır: `setBudgetForPeriod` tek yazma giriş noktasıdır ve hedef dönemle kesişen bütün aktif satırları aynı transaction içinde pasife çeker; `transitionAndSetBudget` kısaltmadan sonra aynı temizliği uygular. `start_date` her zaman dönemin başladığı aydır. Çakışma varsa en yüksek `id` yetkilidir; `getContainingDate` ile `findShadowedBudgetIds` aynı sırayı kullanır. Eski çakışan kayıt silinmez, "çakışıyor" olarak işaretlenip hesaplamadan çıkarılır ve kullanıcı düzeltir. Navigatör mevcut dönemde durur; düzenleme penceresi mevcut + önceki 4 dönemdir, gölgelenmiş kayıt pencere dışında da seçilip silinebilir. `deleteBudget` yalnız hedefi kaldırır, harcamaya dokunmaz. Onboarding kayıtlı döngü gününü yazar | Mevcut çalışma ağacı; `src/utils/budgetPeriodConflicts.ts`, `src/db/budgetDao.ts`, `src/components/BudgetHistoryCard.tsx`, `app/{settings-budget,onboarding}.tsx`, dört dil kaynak/çıktıları, `src/utils/__tests__/budgetPeriodConflicts.test.ts`, `src/db/__tests__/budgetDao.test.ts`, `src/components/__tests__/SettingsBudgetScreen.test.tsx` | `AI-2026-09-03-BUDGET-PERIOD-INTEGRITY-001`: `npm run typecheck` exit 0; çakışma/DAO odaklı 11/11 test, bütçe ekranı 9/9 test; tam Jest 116/116 suite ve 961/961 test geçti; `git diff --check` temiz | Android'de gerçek SQLite üzerinde: çakışan kaydı olan cihazda tek "mevcut" rozeti, gölgelenen kartın işaretlenmesi ve silinebilmesi; döngü günü 1↔23 geçişinde yeni çakışma doğmaması; gelecek dönem kilidi; 5 dönemlik pencere; bütçe silindikten sonra Dashboard/Analiz/bildirimlerin aynı sonucu göstermesi; backup→restore sonrası dönem sınırlarının korunması bekleniyor | `E0` | Jest `expo-sqlite` sürücüsünü mock'lar; SQL kesişim koşulunun gerçek veritabanındaki davranışı, migration ile birlikte çalışması ve mevcut bozuk verinin cihazda gerçekten düzelmesi fiziksel doğrulama gerektirir. Silme geri alınamaz; kullanıcı onayı tek koruma katmanıdır |
| `SPK-UX-023` | Fiyat takibi kartında çok sayfalı liste yalnız yatay kaydırmayla gezilebiliyordu; uzaktaki bir sayfaya ulaşmak aradaki bütün sayfaları teker teker geçmeyi gerektiriyordu ve pasif nokta göstergesi dokunulabilir değildi | Nokta göstergesi, her sayfa için bir rakam taşıyan dokunulabilir şeritle değiştirilir. Rakama dokunmak pager'ı doğrudan o sayfaya animasyonla taşır ve göstergeyi eşitler; programatik kaydırmada momentum olayı beklenmez. Pill genişliği aktif/pasif durumda sabittir, böylece art arda dokunuşta hedef kaymaz. Şerit sığmazsa yatay kayar ve aktif rakam görünür alana çekilir. Liste kısalırsa gösterge son geçerli sayfaya sıkıştırılır. Tek sayfada şerit gösterilmez; kart içi yatay jestin üst sekme gezinmesine devredilmemesi kuralı korunur | Mevcut çalışma ağacı; `src/components/analytics/{PriceWatchCard,analyticsStyles}.tsx`, dört dil kaynak/çıktıları, `src/components/analytics/__tests__/PriceWatchCard.test.tsx` | `AI-2026-09-03-PRICE-PAGE-JUMP-001`: `npm run typecheck` exit 0; odaklı 4/4 test; tam Jest 115/115 suite ve 947/947 test geçti; `git diff --check` temiz | Android'de 2/4/8+ sayfalı listede rakam dokunuşu, şeridin kayması, aktif rakamın görünür kalması; şerit kaydırmasının kart pager'ı ve üst sekme gezinmesiyle çakışmaması; açık/koyu tema, büyük font, dört dil ve TalkBack seçili durum duyurusu bekleniyor | `E0` | Jest sayfa göstergesinin durum değişimini ve şeridin koşullu görünürlüğünü doğrular; `scrollTo` çağrısının gerçek kaydırma sonucu, iç içe yatay jest çakışması ve pill'lerin optik dokunma rahatlığı fiziksel cihaz kabulü gerektirir |
| `SPK-PUB-001` | Uygulamanın Google Play üretim yayınına hazır olup olmadığı tek bir modelin denetimine dayanarak kabul edilmemeli; yayın engelleri, gizlilik beyanı tutarlılığı ve izin gerçeği bağımsız biçimde çapraz doğrulanmalı | İki farklı model (Claude Opus 5 ve GPT-5.6) aynı depoyu birbirinden habersiz, salt-okunur denetler; ardından planlarını karşılıklı inceleyip çelişkileri kapatarak tek plana yakınsar. Model iddiaları kabul edilmeden önce depoda çalıştırılabilir komutla doğrulanır; doğrulanamayan iddia plana “kullanıcı Play Console'dan teyit edecek” sınırlamasıyla girer. Çıktı, 12 aşamalı yayın planı ve Aşama 1'de insana bırakılmış dört karardır. Denetim turlarında hiçbir dosya değiştirilmemiştir | Mevcut çalışma ağacı (commit `a716505`); denetlenen kaynaklar `app.json`, `app.config.js`, `eas.json`, `package.json`, `privacy-policy.html`, `src/services/geminiService.ts`, `assets/`, `.gitignore`; plan çıktısı bu kayıt ve `AI-2026-09-03-PLAY-RELEASE-COUNCIL-001` | `AI-2026-09-03-PLAY-RELEASE-COUNCIL-001`: `npx expo config --type public` çözülmüş izin listesini; `npx expo-doctor` 20 paketlik yama farkını; `npm audit --omit=dev` 36 uyarıyı (1 kritik, 12 yüksek, 22 orta, 1 düşük) ve bunların derleme zincirinde toplandığını; `npm run typecheck` ile 115/115 suite ve 945/945 testin geçtiğini doğruladı | Bekleniyor: Play Console hesap türü/tarihi, 30 Eylül 2026 doğrulama şartı, paket adı kararı; ardından gerçek AAB manifest denetimi, fiziksel cihaz kabulü ve Play pre-launch raporu | `E0` | Bu kayıt yayın planının varlığını belgeler, uygulamanın yayına uygun olduğunu kanıtlamaz. İzin sonucu yalnız çözülmüş Expo yapılandırmasından okunmuştur; kesin kanıt üretilmiş AAB manifestidir. Model kimlikleri kullanıcı beyanına dayanır ve bağımsız doğrulanmamıştır. İki modelin ortak bir hatayı birlikte gözden kaçırma olasılığı bu yöntemle ortadan kalkmaz |
